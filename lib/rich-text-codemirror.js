...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.onAnnotationsChanged_ = function(oldNodes, newNodes) {
    ...
    for (i = 0; i < newNodes.length; i++) {
      ...
      if (forLine) {
        ...
        var hrefAttributes = this.getAttributesForHref_(annotation.attributes);

        if (className !== '') {
          var to = this.codeMirror.posFromIndex(newNodes[i].pos + newNodes[i].length);
          marker = this.codeMirror.markText(from, to, { className: className, attributes: hrefAttributes });
  ...
  RichTextCodeMirror.prototype.getAttributesForHref_ = function(attributes) {
    for (var attr in attributes) {
      if (attr === ATTR.HREF) {
        return {
          'data-href': attributes[attr],
          'onclick': `window.open("${attributes[attr]}")`,
        };
      }
    }
  ...
  RichTextCodeMirror.prototype.onCodeMirrorChange_ = function(cm, cmChanges) {
    ...
    for (var i = 0; i < changes.length; i++) {
      ...
      if (text.length > 0) {
        ...
        // @NOTE
        // Where the annotated attributes are set into the span
