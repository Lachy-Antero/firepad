...
firepad.RichTextToolbar = (function(global) {
  ...
  function RichTextToolbar(imageInsertionUI, hyperlinkUI) {
  ...
  utils.makeEventEmitter(RichTextToolbar, ['bold', 'italic', 'underline', 'strike', 'font', 'font-size', 'color',
    'left', 'center', 'right', 'unordered-list', 'ordered-list', 'todo-list', 'indent-increase', 'indent-decrease',
                                           'hyper-link', 'undo', 'redo', 'insert-image']);
  ...
  RichTextToolbar.prototype.makeElement_ = function() {
    ...
    if (self.imageInsertionUI) {
      insertUIButtons.push(self.makeButton_('insert-image'));
    }

    if (self.hyperlinkUI) {
      insertUIButtons.push(self.makeButton_('hyper-link'))
    }

    if (insertUIButtons.length) {
      toolbarOptions.push(utils.elt('div', insertUIButtons, { 'class': 'firepad-btn-group' }));
    }
